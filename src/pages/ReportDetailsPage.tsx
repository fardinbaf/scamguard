...
import { Report, ReportStatus, Comment as CommentType, User, EvidenceFile } from '../types';
import { 
  getReportById, 
...
import { REPORT_STATUS_OPTIONS, TrashIcon, CommentIcon as CommentsSectionIcon, AttachmentIcon } from '../constants';
...
const CommentEntry: React.FC<CommentEntryProps> = ({ comment }) => {
  ...
  const displayName = comment.isAnonymous ? "Anonymous" : comment.userIdentifier;
  return (
...
const ReportDetailsPage: React.FC = () => {
  ...
  const fetchReportAndComments = useCallback(async () => {
    ...
    try {
      ...
      if (fetchedReport) {
        if (!isAdmin && fetchedReport.status !== ReportStatus.APPROVED) {
          setError("This report is not currently public or has been removed.");
          setReport(null);
        } else {
          setReport(fetchedReport);
          if (fetchedReport.status === ReportStatus.APPROVED) {
  ...
  const handleStatusChange = async (newStatus: ReportStatus) => {
    ...
    try {
      ...
      if (updatedReport) {
        ...
        if (updatedReport.status === ReportStatus.APPROVED) {
          const fetchedComments = await getCommentsByReportId(reportId);
          setComments(fetchedComments);
        } else {
          setComments([]); 
  ...
  const handleDeleteReport = async () => {
    if (!reportId || !report || !isAdmin) return;
    if (window.confirm(`Are you sure you want to delete the report "${report.title}"? This action cannot be undone and will also remove all associated comments.`)) {
      try {
        const success = await serviceDeleteReport(reportId);
        if (success) {
          alert(`Report "${report.title}" deleted successfully.`);
          navigate(isAdmin ? '/admin' : '/search'); 
        } else {
          alert(`Failed to delete report "${report.title}".`);
        }
      } catch (error) {
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const addedComment = await serviceAddComment(reportId, newCommentText, currentUser, postAnonymously);
      setComments(prevComments => [...prevComments, addedComment]);
      setNewCommentText('');
      setPostAnonymously(false); // Reset checkbox
    } catch (err) {
  ...
  if (!report) {
    return (
        <div className="container mx-auto px-4 py-8 text-center">
            <p className="text-gray-600 text-xl">Report not available or access denied.</p>
            <Link to="/" className="mt-4 inline-block bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
             Go to Homepage
            </Link>
        </div>
  ...
  const timeAgo = (timestamp: number): string => new Date(timestamp).toLocaleDateString() + ' ' + new Date(timestamp).toLocaleTimeString();
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white shadow-xl rounded-lg p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 mb-6 text-sm text-gray-600">
          ...
          <p><strong>Reported By:</strong> <span className="font-mono text-gray-700">{report.reporterIdentifier || report.reportedBy}</span></p>
          <p><strong>Reported On:</strong> {timeAgo(report.createdAt)}</p>
        </div>
        ...
        {report.evidenceFiles && report.evidenceFiles.length > 0 && (
          <div className="mb-6">
            <h2 className="text-xl font-semibold text-gray-700 mb-2 flex items-center">
              <AttachmentIcon className="w-5 h-5 mr-2 text-gray-600"/> Evidence Attachments
            </h2>
              ...
              {report.evidenceFiles.map((file, index) => (
                <li key={index} className="p-2 border rounded-md bg-gray-50">
                  ...
                  <div className="text-xs text-gray-500">
                    Type: {file.type} | Size: {formatBytes(file.size)}
                  </div>
                  {file.dataUrl && file.type.startsWith('image/') && (
                    <img src={file.dataUrl} alt={`Evidence preview ${index + 1}`} className="mt-2 max-w-xs max-h-48 rounded shadow"/>
                  )}
                </li>
              ))}
            </ul>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center space-x-2">
                ...
                <select
                  ...
                  className="px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
      ...
      {report.status === ReportStatus.APPROVED && (
        <div className="mt-8 bg-white shadow-xl rounded-lg p-6 md:p-8">
          <h2 className="text-2xl font-semibold text-gray-800 mb-4 flex items-center">
            <CommentsSectionIcon className="w-6 h-6 mr-2 text-blue-600" /> Community Comments
          </h2>
          {comments.length > 0 ? (
          ...
          {isAuthenticated && currentUser ? (
            <form onSubmit={handleCommentSubmit}>
              ...
              <textarea
                ...
                placeholder="Share your experience or opinion related to this report..."
                required
              ...
              <div className="mt-2 flex items-center">
                ...
                />
                <label htmlFor="postAnonymously" className="ml-2 block text-sm text-gray-700">
                  Post anonymously
                </label>
              </div>
              <button
                ...
                className="mt-3 flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50"
              >
            ...
            <p className="text-gray-500">
              <button onClick={() => { const loginButton = document.querySelector('header button > svg[data-icon="login"]'); if (loginButton) (loginButton.closest('button') as HTMLElement)?.click(); }} className="text-blue-600 hover:underline font-medium">Login</button> to add a comment.
            </p>
          )}
        </div>
      )}
