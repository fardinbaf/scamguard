...
const ReportDetailsPage: React.FC = () => {
  ...
  const fetchReportAndComments = useCallback(async () => {
    ...
    try {
      ...
      if (fetchedReport) {
        ...
        // Comments are fetched only if the report is approved
  ...
  const handleStatusChange = async (newStatus: ReportStatus) => {
    ...
    try {
      ...
      if (updatedReport) {
        ...
        if (updatedReport.status === ReportStatus.APPROVED && comments.length === 0) {
          // Re-fetch comments if it just got approved and comments were not loaded before
          const fetchedComments = await getCommentsByReportId(reportId);
          setComments(fetchedComments);
        } else if (updatedReport.status !== ReportStatus.APPROVED) {
          setComments([]); 
        }
      }
    } catch (err) {
  ...
  const timeAgo = (timestamp: number): string => new Date(timestamp).toLocaleString();
  ...
  if (!report) return (
    <div className="container mx-auto px-4 py-8 text-center">
      <p className="text-gray-600 text-xl">Report not found.</p>
      <Link to="/" className="mt-4 inline-block bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Go to Homepage</Link>
    </div>
  );
            ...
            <ul className="space-y-2">
              {report.evidenceFiles.map((file) => (
                <li key={file.id} className="p-2 border rounded-md bg-gray-50">
                  <a href={`${API_URL}/${file.filePath}`} target="_blank" rel="noopener noreferrer" className="font-medium text-blue-600 hover:underline">{file.originalName}</a>
                  <div className="text-xs text-gray-500">
                    Type: {file.mimeType} | Size: {formatBytes(file.size)}
                  </div>
                   {file.mimeType && file.mimeType.startsWith('image/') && (
                    <img src={`${API_URL}/${file.filePath}`} alt={`Evidence preview for ${file.originalName}`} className="mt-2 max-w-xs max-h-48 rounded shadow"/>
                  )}
                </li>
              ))}
            </ul>
