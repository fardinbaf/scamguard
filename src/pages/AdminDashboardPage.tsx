...
import { REPORT_STATUS_OPTIONS, TrashIcon } from '../constants';
...
const AdminDashboardPage: React.FC = () => {
  ...
  const [filters, setFilters] = useState<ReportFilters>({
    ...
    status: "All Statuses", // Admin can see all by default or filter
  });

  const fetchAllReports = useCallback(async () => {
    setIsLoading(true);
    try {
      // Admins should fetch all reports, respecting their filters
  ...
  const handleDeleteReport = async (reportId: string, reportTitle: string) => {
    if (window.confirm(`Are you sure you want to delete the report "${reportTitle}"? This action cannot be undone and will also remove all associated comments.`)) {
      try {
        ...
        if (success) {
          ...
          alert(`Failed to delete report "${reportTitle}".`);
        }
      } catch (error) {
  ...
  if (!currentUser || !isAdmin) {
     return <div className="text-center p-8">Access Denied. You must be an administrator.</div>;
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {reports.length === 0 && !isLoading ? (
        <p className="text-center text-gray-500 py-10">No reports match your criteria or no reports submitted yet.</p>
      ) : (
